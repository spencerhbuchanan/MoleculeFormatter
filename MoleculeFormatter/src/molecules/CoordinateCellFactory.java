 ...
 * Creates a cell factory to be used in the Molecule table which can be edited either by a spinner 
 * or by direct typing.
 * 
 * A majority of these methods overwrite Cell methods, so they already have Javadoc's
 * 
...
public class CoordinateCellFactory extends TableCell<molecules.Atom, Number> {
  ...
  protected void updateItem(Number item, boolean empty) {
    ...
    if (item != null) {
      ...
      setTextFill(isSelected() ? Color.WHITE
                               : value == 0 ? Color.BLACK : value < 0 ? Color.RED : Color.GREEN);
    }
  }
  ...
  public void cancelEdit() {
    ...
    /*
     * Needs to be done like this otherwise it doesn't work. Mysteriously, lets it reset to previous
     * value....
     */
  ...
  private void createCoordinateSpinner() {
    //Creates a new Double spinner factory, with a min/max of the min/max of Double
    DoubleSpinnerValueFactory doubleSpinnerFactory =
                                                   new DoubleSpinnerValueFactory(-Double.MAX_VALUE,
                                                                                 Double.MAX_VALUE);
